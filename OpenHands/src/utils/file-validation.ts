...
const MAX_TOTAL_SIZE = 100 * 1024 * 1024; // 100MB maximum total size for all files combined
...
export function validateTotalFileSize(
  ...
  if (totalSize > MAX_TOTAL_SIZE) {
    ...
    return {
      isValid: false,
      errorMessage: `Total file size would be ${totalSizeMB}MB, exceeding the 100MB limit. Please select fewer or smaller files.`,
    };
  }
