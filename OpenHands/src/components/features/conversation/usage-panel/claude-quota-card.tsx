...
import { ClaudeRateLimitWindow } from "#/api/claude-usage-service.types";
...
interface WindowMeterProps {
  label: string;
  windowData: ClaudeRateLimitWindow;
}
  ...
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <div className="relative h-2 w-full rounded-full bg-tertiary overflow-hidden">
        <div
          data-testid="claude-quota-meter-bar"
          className={cn(
...
export function ClaudeQuotaCard() {
  const { t } = useTranslation("openhands");
  ...
  const isCodexConversation =
    (conversation?.agent_kind === "acp" &&
      (conversation?.acp_server === "codex" ||
        conversation?.tags?.acpserver === "codex")) ||
    (typeof conversation?.llm_model === "string" &&
      (conversation.llm_model.toLowerCase().includes("codex") ||
        conversation.llm_model.toLowerCase().includes("gpt") ||
        conversation.llm_model.toLowerCase().includes("o3") ||
        conversation.llm_model.toLowerCase().includes("o4")));

  if (!quota || isCodexConversation) {
    return null;
  }
  ...
  const planName = quota.planType?.toLowerCase().includes("team")
    ? "Claude Team"
    : quota.planType?.toLowerCase().includes("max")
      ? "Claude Max"
      : quota.planType?.toLowerCase().includes("api")
        ? "Claude API"
        : "Claude Pro";

  return (
    <div
      data-testid="claude-quota-card"
      ...
      <div className="flex items-center justify-between pb-1 border-b border-[var(--oh-border-subtle)]">
        <div className="flex items-center gap-2">
          ...
          <span className="font-semibold text-sm">
            {t(I18nKey.CONVERSATION$CLAUDE_QUOTA)}
          </span>
        </div>
        <div className="flex items-center gap-2">
