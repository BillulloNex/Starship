...
export default function FuelGaugeSettingsScreen() {
  ...
  const { limits, worstStatus, isAnyExhausted, isFetching, refetch } =
    useUnifiedLimits();
  ...
  return (
    <div
      data-testid="fuel-gauge-settings-screen"
    ...
    >
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-[var(--oh-muted)] mt-0.5">
              Provider-reported quota, rate limits, token usage, and tracked
              balances where each service supports them.
            </p>
          </div>
        </div>
        ...
        <div className="flex items-start gap-2.5 p-3.5 rounded-lg bg-red-500/10 border border-red-500/20 text-red-400 text-xs">
          ...
          <div>
            <span className="font-semibold">
              One or more providers have exhausted their quota.
            </span>
            <p className="mt-0.5 text-red-400/90">
              Check your provider billing dashboard or switch active models in
              LLM settings.
            </p>
          </div>
        </div>
      ...
      <section className="flex flex-col gap-3 p-5 rounded-xl border border-[var(--oh-border)] bg-surface-raised shadow-xs">
        <div className="flex items-center justify-between border-b border-[var(--oh-border-subtle)] pb-3">
          <div>
            <Typography.H3 className="text-sm font-semibold text-[var(--oh-foreground)]">
              Connected Providers & Usage
            </Typography.H3>
            <p className="text-xs text-[var(--oh-muted)] mt-0.5">
              Rate limits, reset timers, token totals, and balances detected
              from active services.
            </p>
          </div>
          <span className="text-xs font-medium text-[var(--oh-muted)]">
            {limits.length} {limits.length === 1 ? "provider" : "providers"}{" "}
            tracked
          </span>
        </div>
        ...
        <div className="pt-1">
          {limits.length > 0 ? (
            ...
            <div className="py-8 text-center text-xs text-[var(--oh-muted)]">
              ...
              <br />
              Configured API keys will automatically appear as quota telemetry
              is polled.
            </div>
          )}
        </div>
      ...
      <section className="flex flex-col gap-3 p-5 rounded-xl border border-[var(--oh-border)] bg-surface-raised shadow-xs">
        <div className="border-b border-[var(--oh-border-subtle)] pb-3">
          ...
          <p className="text-xs text-[var(--oh-muted)] mt-0.5">
            Add pay-as-you-go balance pools (e.g. OpenAI, Anthropic, OpenRouter,
            DeepSeek). Grokbot automatically tracks and decrements balance as
            conversation tokens accumulate.
          </p>
        </div>
      ...
      <div className="flex items-start gap-2.5 p-4 rounded-xl bg-surface-base border border-[var(--oh-border-subtle)] text-xs text-[var(--oh-muted)]">
        ...
        <div className="space-y-1">
          <span className="font-semibold text-[var(--oh-foreground)]">
            How Auto-Decrement Works
          </span>
          <p>
            When conversations run, token usage and cost metrics are observed in
            the background to decrement your estimated remaining credits in real
            time. Updating manual credit entries recalibrates any estimation
            drift.
          </p>
        </div>
      </div>
