...
13. Preserve and label every generated result. Report every label, IP direction and rationale, prompt/color mapping, and dimensions. In your final response, ALWAYS embed every generated candidate as an inline markdown image `![Label — description](saved_path)` so the user can visually see and compare all candidates directly in the chat. Never output raw container hyperlinks like `[Label](/home/openhands/...)` that lack image embeds. Present all results together; generate refinements or replacements only when the user explicitly asks for another draw.
...
- In your final response message to the user, ALWAYS embed all generated images using standard markdown image syntax `![Label — details](<saved_path>)` so all generated images appear directly as inline visual cards in the chat feed.
